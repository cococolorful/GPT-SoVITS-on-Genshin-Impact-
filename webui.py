...
# 添加外部参数
parser.add_argument('--model_data_path', type=str, default='xxx')

# 解析外部参数
args = parser.parse_args()

model_data_path = args.model_data_path
model_name_whole = os.path.splitext(os.path.basename(model_data_path))[0]
...
with gr.Blocks(title="GPT-SoVITS WebUI") as app:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem(i18n("1-GPT-SoVITS-TTS")):
            with gr.Row():
                exp_name = gr.Textbox(label=i18n("*实验/模型名"), value=model_name_whole, interactive=True)
            ...
            with gr.TabItem(i18n("1A-训练集格式化工具")):
                ...
                print(os.path.dirname(model_data_path))
                with gr.Row():
                    inp_text = gr.Textbox(label=i18n("*文本标注文件"),value=model_data_path ,interactive=True)
                    inp_wav_dir = gr.Textbox(
                        label=i18n("*训练集音频文件目录"),
                        value=os.path.dirname(model_data_path),
                        interactive=True,
